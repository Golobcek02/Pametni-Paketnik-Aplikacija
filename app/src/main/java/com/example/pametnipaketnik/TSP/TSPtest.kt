...
object TSPTest {
    ...
    fun main(args: Array<String>) {
        ...
        for (i in 0..29) {
            val eilTsp = TSP("src/main/resources/bays29.tsp", 1000000)
            ...
            stats.add("neke",bestPath.distance)
            list.add(bestPath.distance)
            // shrani min, avg in std
        }
        println(RandomUtils.getSeed()) // izpiše seme s katerim lahko ponovimo zagon

        stats.dictionary.forEach() {
            println("${it.key} ${it.value.forEach { i-> println(i) }}")
        }
        println()
        println("-------------------")
        list.forEach { println(it) }
        println("-------------------")
        println(stats.returnStatisticsForProblem("neke"))*/
